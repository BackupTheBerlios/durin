package Durin::Components::IO::DataStandard;
...
# We cannot write, we need to know the class to write.
sub write
  {
    my ($class,$disp,$object) = @_;

    my $XMLGen = new XML::Generator;

    $disp->print("<?xml version="1.0"?>\n");
    $object->writeDocType($disp);
    $object->writeMetadata($disp);
    $object->writeData($disp);
    #$disp->print("<!DOCTYPE DataStandard SYSTEM "DataStandard.dtd">");
    #$disp->print($XMLGen->Type("","DataStandard"));
    #$disp->print($XMLGen->("","DataStandard")
