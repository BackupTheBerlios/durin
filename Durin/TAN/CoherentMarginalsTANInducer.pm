package Durin::TAN::CoherentMarginalsTANInducer;
...
use Durin::ProbClassification::ProbApprox::PAMarginals;
...
sub run
{
  ...
  my $inducer = $self->{INDUCER};
  {
    ...
    $input->{TAN}->{PROBAPPROX} = Durin::ProbClassification::ProbApprox::PAMarginals->new();
...
sub getDetails {
  ...
  $details->{"Probability approximation for TAN"} = "PAMarginals";
