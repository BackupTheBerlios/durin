...
package Durin::TAN::FrequencyHierarchicalMarginalsTANInducer;
...
use Durin::ProbClassification::ProbApprox::PAHierarchicalMarginals;
...
sub run
{
  ...
  my $inducer = $self->{INDUCER};
  {
    ...
    $input->{TAN}->{PROBAPPROX} = Durin::ProbClassification::ProbApprox::PAHierarchicalMarginals->new();
...
sub getDetails {
  ...
  $details->{"Probability approximation for TAN"} = "PAHierarchicalMarginals";
