# This class implements the Naive Bayes with Prob Approx as described in Webb paper for AODE

package Durin::ProbClassification::ProbApprox::PALaplace2;
...
sub getPXClass {
  ...
  my $LaplaceAddition = $self->{COUNTTABLE}->getNumClasses() * $self->{COUNTTABLE}->getNumAttValues($attX);
  
  return ($self->{COUNTTABLE}->getCountXClass($classVal,$attX,$attXVal) + 1) / ($self->{COUNTTABLE}->getCount() + $LaplaceAddition);
...
sub getPXCondClass
  {
    ...
    return $self->getPXClass($classVal,$attX,$attXVal) / $self->getPClass($classVal);
