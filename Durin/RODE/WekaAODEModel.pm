package Durin::RODE::WekaAODEModel;
...
sub predict {
  ...
  if ($numModelsApplied == 0) {
	...
	if ($node_u != $class_attno)  {
	  ...
	  my $den = $ct->getCountClass($class_val) + $numValues_u;
	  $Prob{$class_val} *= $num / $den;
	  #($num / ($den * $pThisClass));
	}
      }
    }
...
sub computeProbConcreteModel {
  ...
  for(my $node_v = 0 ; $node_v < $num_atts; $node_v++) {
    if (($node_v != $class_attno) && ($node_v != $node_u)) { 
      ...
      my $p_v_cond_uc = ($ct->getCountXYClass($class_val,$node_u,$u_val,$node_v,$v_val) + 1) / ($N_uc +  $numValues_v);
      $prob *= $p_v_cond_uc;
    }
  }
  return $prob;
